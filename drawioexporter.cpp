...
// Стиль внешней стрелки-входа (стрелка «от внешнего мира»):
// startArrow=classic → стрелка на конце, идущем К блоку
// endArrow=none — хвост без наконечника
static const QString EXT_IN_STYLE =
    "edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;"
    "startArrow=classic;startFill=1;endArrow=none;endFill=0;";

// Стиль внешней стрелки-выхода (стрелка «от блока наружу»):
static const QString EXT_OUT_STYLE =
    "edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;"
    "startArrow=none;startFill=0;endArrow=classic;endFill=1;";

// Стиль межблочного edge (из выхода одного в вход другого):
...
{
    QString cells;
    QString pageRoot = "page_root";  // parent для всех ячеек

    // ── Карта: blockId → cellId (mxCell id прямоугольника блока)
    QMap<QString,QString> cellIdMap;

    // ── 1. Рисуем прямоугольники функциональных блоков ──────────────────
    for (const Idef0Block &b : diagram.blocks) {
        ...
        // Метка: имя блока (без id — как в reference XML)
        QString label = xe(b.name);
        ...
        cells += QString(
                     ...
                     "</mxCell>\n"
                     ).arg(cid, label, BLOCK_STYLE, pageRoot)
                     .arg(b.x).arg(b.y).arg(b.w).arg(b.h);

        // Номер блока в нижнем правом углу (правило 2 гл.11 Марка)
    ...
    // ── 2. Межблочные соединения: выход → вход (совпадающие метки) ──────
    // Делаем edge между source и target блоками — draw.io сам прокладывает маршрут.
    // Метка ставится по центру edge.
    //
    // Также строим множества «уже соединённых» стрелок, чтобы не дублировать
    // их как внешние.
    QSet<QString> connectedInputs;    // "blockId:arrowLabel"
    QSet<QString> connectedOutputs;

    for (const Idef0Block &src : diagram.blocks) {
        if (src.parentId.isEmpty() || !cellIdMap.contains(src.id)) continue;
        for (int oi = 0; oi < src.outputs.size(); ++oi) {
            const Idef0Arrow &out = src.outputs[oi];
            for (const Idef0Block &tgt : diagram.blocks) {
                if (tgt.id == src.id || tgt.parentId.isEmpty()) continue;
                if (!cellIdMap.contains(tgt.id)) continue;
                for (int ii = 0; ii < tgt.inputs.size(); ++ii) {
                    if (out.label == tgt.inputs[ii].label) {
                        // Точки подключения: выход — правая грань src, вход — левая tgt
                        // (координаты не используются напрямую — draw.io строит маршрут
                        //  по exitX/entryX; вычисляем только для возможной отладки)
                        [[maybe_unused]] auto [sx, sy] = connPoint(src, ArrowPoint::Right, oi, src.outputs.size());
                        [[maybe_unused]] auto [tx, ty] = connPoint(tgt, ArrowPoint::Left,  ii, tgt.inputs.size());

                        QString eid = uid(id, "e_");

                        // exitX/Y по позиции на правой грани src
                        ...
                        cells += QString(
                                     ...
                                     .arg(exitY,  0, 'f', 4)
                                     .arg(entryY, 0, 'f', 4)
                                     .arg(cellIdMap[src.id])
                                     .arg(cellIdMap[tgt.id])
                                     .arg(pageRoot);
    ...
    // ── 3. Внешние стрелки ──────────────────────────────────────────────
    // По правилам SADT:
    //   Input  → левая сторона первого блока (A1) или самого левого
    //   Output → правая сторона последнего блока (An) или самого правого
    //   Control → верхняя сторона
    //   Mechanism → нижняя сторона
    //
    // Стиль: линия со стрелкой, идущей ОТ края страницы К блоку (input/control)
    //        или ОТ блока К краю страницы (output/mechanism).
    //
    // Метка располагается как label на edge (параметр value) —
    // draw.io сам её позиционирует.

    for (const Idef0Block &b : diagram.blocks) {
        if (b.parentId.isEmpty() || !cellIdMap.contains(b.id)) continue;
        const QString &cid = cellIdMap[b.id];

        // ── INPUT (левая грань, стрелка приходит слева) ──────────────────
        ...
        for (int i = 0; i < nIn; ++i) {
            ...
            // Источник — точка на левом краю рабочей зоны
            int srcX = EXT_LEFT + 10;
            int srcY = by;
            ...
            cells += QString(
                         ...
                         .arg(EXT_IN_STYLE)
                         .arg(entryY, 0, 'f', 4)
                         .arg(cid).arg(pageRoot)
                         .arg(srcX).arg(srcY);
        }

        // ── OUTPUT (правая грань, стрелка уходит вправо) ─────────────────
        ...
        for (int i = 0; i < nOut; ++i) {
            ...
            int dstX = EXT_RIGHT - 10;
            int dstY = by;

            QString eid = uid(id, "e_");
            // Выходные стрелки: exitX=1, цель — точка на правом краю
            cells += QString(
                         "<mxCell id=\"%1\" value=\"%2\" "
                         "style=\"%3"
                         "exitX=1;exitY=%4;exitDx=0;exitDy=0;"
        ...
        // ── CONTROL (верхняя грань, стрелка приходит сверху) ─────────────
        ...
        for (int i = 0; i < nCtl; ++i) {
            ...
            cells += QString(
                         ...
                         "exitX=%5;exitY=0;exitDx=0;exitDy=0;"
                         "fontSize=14;align=left;verticalAlign=bottom;\" "
                         "edge=\"1\" target=\"%6\" parent=\"%7\">\n"
                         "  <mxGeometry relative=\"1\" as=\"geometry\">\n"
        ...
        // ── MECHANISM (нижняя грань, стрелка приходит снизу) ─────────────
        ...
        for (int i = 0; i < nMec; ++i) {
            ...
            QString eid = uid(id, "e_");
            // Механизм: стрелка снизу вверх — к нижней грани блока
            // startArrow=none (от низа страницы), endArrow=classic (в блок)
            cells += QString(
                         "<mxCell id=\"%1\" value=\"%2\" "
                         "style=\"%3"
